...
import type { FnCall, FuncArg, Index, IndexM, Sym, Term, Type } from '@thi.ng/shader-ast';
...
import { GLSLOpts, GLSLTarget, GLSLVersion } from './Target.types';
...
export const targetGLSL = (opts?: Partial<GLSLOpts>) => {
        ...
        if (opts.type) {
            ...
            if (_opts.version < GLSLVersion.GLES_300) {
                if (isVS) {
                    ...
                    !type && unsupported("GLSL 100 doesn't support fragment shader output variables");
                }
            } else {
    ...
    const emitIndex = (t: Index<any> | IndexM<any>) => `${emit(t.val)}[${emit(t.id)}]`;
    ...
    const emit: Fn<Term<any>, string> = defTarget({
        ...
        call_i: t =>
            t.id === 'texture' && _opts.version < GLSLVersion.GLES_300
                ? `${t.id}${(<Sym<any>>t.args[0]).type.substring(7)}(${$list(t.args)})`
                : $fn(t),
    ...
    ${t.body}
        ...
        scope: t => {
            let res = t.body
                .map(emit)
                .reduce((acc, x) => (acc.push(RE_SEMI.test(x) ? x : x + ';'), acc), <string[]>[])
                .join('\n');
