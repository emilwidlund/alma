...
export class Texture implements ITexture {
    ...
    configure(opts: Partial<TextureOpts> = {}, unbind = true) {
        ...
        opts.flip !== undefined && gl.pixelStorei(gl.UNPACK_FLIP_Y_WEBGL, opts.flip ? 1 : 0);

        opts.premultiply !== undefined && gl.pixelStorei(gl.UNPACK_PREMULTIPLY_ALPHA_WEBGL, opts.premultiply ? 1 : 0);
    ...
    protected configureImage(target: TextureTarget, opts: Partial<TextureOpts>) {
    ...
    protected configureImage2d(target: TextureTarget, opts: Partial<TextureOpts>) {
        ...
        if (width && height) {
            if (opts.sub) {
                ...
                gl.texImage2D(target, level, format, width, height, 0, baseFormat, type, <ArrayBufferView>image);
            }
        } else {
            if (opts.sub) {
                gl.texSubImage2D(target, level, pos[0], pos[1], baseFormat, type, <TexImageSource>image);
            } else {
                if (image != null && level === 0) {
                    this.size = [(<any>image).width, (<any>image).height];
                }
                gl.texImage2D(target, level, format, baseFormat, type, <TexImageSource>image);
            }
        }
    }

    protected configureImage3d(target: TextureTarget, opts: Partial<TextureOpts>) {
    ...
    protected configureFilter(target: TextureTarget, opts: Partial<TextureOpts>) {
        const gl = this.gl;
    ...
    protected configureWrap(target: TextureTarget, opts: Partial<TextureOpts>) {
        ...
        isGL2Context(gl) &&
            ...
            gl.texParameteri(target, (<WebGL2RenderingContext>gl).TEXTURE_WRAP_R, t3);
    }
    ...
    protected configureLOD(target: TextureTarget, opts: Partial<TextureOpts>) {
        ...
        if (opts.lod) {
            ...
            t1 && gl.texParameterf(target, (<WebGL2RenderingContext>gl).TEXTURE_MIN_LOD, t1);
            t2 && gl.texParameterf(target, (<WebGL2RenderingContext>gl).TEXTURE_MAX_LOD, t2);
        }
    }

    protected configureLevels(target: TextureTarget, opts: Partial<TextureOpts>) {
        const gl = this.gl;
        if (opts.minMaxLevel) {
            const [t1, t2] = opts.minMaxLevel;
            gl.texParameteri(target, (<WebGL2RenderingContext>gl).TEXTURE_BASE_LEVEL, t1);
            gl.texParameteri(target, (<WebGL2RenderingContext>gl).TEXTURE_MAX_LEVEL, t2);
        }
    }
...
export const defTextureCubeMap = (
    ...
    const tex = new Texture(gl, undefined, { target: gl.TEXTURE_CUBE_MAP });
    const faceOpts = withoutKeysObj(opts, ['target', 'image', 'filter', 'mipmap']);
    for (let i = 0; i < 6; i++) {
